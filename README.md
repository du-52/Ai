...
`stmUIForAllModel.py` is the frontend code (using Streamlit). It loads 3 NLP models from the 3 `.joblib` files located in the same directory. `requirements.txt` contains version information of all dependencies, and must be in the same directory as `stmUIForAllModel.py`.
